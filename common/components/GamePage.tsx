...
@template(function () {
  const game = val(this.properties.game);
  return (
    <div>
      <a href={`/${game.id}`} target="_blank">Open new tab</a>
      <span>
        {always(() =>
          game.turn === Datex.Runtime.endpoint ? "Your turn" : "Other's turn"
        )}!
      </span>
...
export class GamePage extends Component<{ game: Ref<GameType> }> {
  ...
  private set(index: number) {
    ...
    if (game.turn === Datex.Runtime.endpoint) {
      ...
      if (game.map.get(index) === "") {
        ...
        const other = [...game.players].find((p) =>
          p != Datex.Runtime.endpoint
        );
        game.turn = other;
      }
    } else {
  ...
  protected override onDisplay() {
    ...
    map.observe(() => {
      if (this.isRunning) {
        const host = val(this.properties.game).host === Datex.Runtime.endpoint;
    ...
    console.log("Game pointer", this.properties.game);
  }
}
